...
pub enum Opcode {
    ...
    Sign,
    /// The opcode is for a portable hash operation (i.e. `hash.portable.keccak256`).
    PortableHash(&'static str),
}
...
impl Deref for Opcode {
    ...
    fn deref(&self) -> &Self::Target {
        match self {
            ...
            Opcode::PortableHash(opcode) => opcode,
        }
    }
}
...
impl Display for Opcode {
    ...
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        match self {
            ...
            Self::PortableHash(opcode) => write!(f, "{opcode}"),
        }
    }
}
